...
from pprint import pprint  # Add this import
...
class ExpTester(AbstractTrainer):
    ...
    def _test_settings(self, model_cfg=None, data_cfg=None, config_cfg=None):
        ...
        # Load model
        model_cfg = {k: v for k, v in model_cfg.items() if k != 'name'}  # Added this line to remove 'name' from model_cfg
        self.num_classes = model_cfg["num_classes"]
        self.model = load_model(self.model_name)(**model_cfg)  # No 'name' argument is passed here
    ...
    def _load_ckpt(self, best=False, train=False):
        load_dir = os.path.join(self.dir, self.ckpt + ".pt" if best else "latest_model.pt")
        load_dict = torch.load(load_dir, map_location=self.device)
        self.start_step = load_dict.get("step", 0)
        self.best_step = load_dict.get("best_step", 0)
        self.best_metric = load_dict.get("best_metric", 0.0)
        ...
        print(f"Loading checkpoint from {load_dir}, best step: {self.best_step}, best {self.eval_metric}: {round(self.best_metric, 4)}.")
    ...
    def test(self, display_images=False):
        ...
        # Ensure the test dataset has categories or handle the lack of this attribute
        categories = getattr(self.test_loader.dataset, 'categories', None)
        for idx, test_data in test_generator:
            ...
            if self.num_classes == 1:
                ...
                Y_pre = torch.softmax(Y_pre, dim=1)[:, 1]  # Extract probabilities for the positive class
            Y_labels = Y.argmax(dim=1)  # Convert one-hot to class labels
            acc.update(Y_pre.round(), Y_labels)  # Use rounded predictions
            auc.update(Y_pre, Y_labels)
            ...
            if display_images and idx == t_idx:
                ...
                images = I[:4]
                pred = Y_pre[:4]
                gt = Y_labels[:4]  # Show class labels
                self.plot_figure(images, pred, gt, 2, categories)
        print("Test, FINAL LOSS %.4f, FINAL ACC %.4f, FINAL AUC %.4f" % (logloss.avg, acc.mean_acc(), auc.compute_auc()))
